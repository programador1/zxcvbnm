...
/**
 * width - element width (by default 512px) json_url - url to fetch json object
 * cache - use cache height - maximum number of element shown before scroll will
 * apear newel - show typed text like a element firstselected - automaticly
 * select first element from dropdown filter_case - case sensitive filter
 * filter_selected - filter selected items from list filter_begin - filter only
 * from begin complete_text - text for complete page maxshownitems - maximum
 * numbers that will be shown at dropdown list (less better performance)
 * oncreate - fire event on item create onselect - fire event on item select
 * onremove - fire event on item remove maxitimes - maximum items that can be
 * added delay - delay between ajax request (bigger delay, lower server time
 * request) addontab - add first visible element on tab or enter hit addoncomma -
 * add first visible element when pressing the comma key attachto - after this
 * element fcbkcomplete insert own elements bricket - use square bricket with
 * select (needed for asp or php) enabled by default input_tabindex - the
 * tabindex of the input element input_min_size - minimum size of the input
 * element (default: 1) input_name - value of the input element's
 * 'name'-attribute (no 'name'-attribute set if empty) select_all_text - text
 * for select all link
 */
      ...
      function createFCBK() {
        ...
        if (options.complete_text != "") {
          ...
          if (options.select_all_text) {
        	  complete.children('.default').append($('<button id="nueva_persona" name="nueva_persona" type="button">' + options.select_all_text + '</button>'));
          }
        }
          ...
          if (options.cache && json_cache_object.get(etext)) {
            ...
            setTimeout( function() {
              ...
              $.getJSON(options.json_url, {"tag": xssDisplay(etext)}, function(data) {
                if (!isactive) return; // prevents opening the selection again
										// after the focus is already off
