...
class modelo_patente_regional extends CI_Model {
    ...
    function personas() {
        $sql = "SELECT 
                ...
                FROM 
                        persona AS per
                WHERE TRIM(COALESCE(per.nombre_persona,NULL,'')||' '||COALESCE(per.paterno_persona,NULL,'')||' '||COALESCE(per.materno_persona,NULL,'')||' '||COALESCE(per.apellido_casada,NULL,'')) <> '' OR 
    ...
    function minerales() {
        $sql = "SELECT 
                        min.id_mineral AS key, nombre AS value
                FROM 
                        mineral AS min
                ORDER BY nombre";
        //return consulta($sql);
