...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$default_per_page = $this->config->default_per_page;
		$data->paging_options = array('5','10','25','50','100');
	...
	protected function get_true_false_input($field_info,$value)
	{
		...
		$value_is_null = empty($value) && $value !== 'I' && $value !== 'I' ? true : false;
		
		$input = "<div class='pretty-radio-buttons'>";
		
		$checked = $value === 'A' || ($value_is_null && $field_info->default === 'A') ? "checked = 'checked'" : "";
		$input .= "<label><input id='field-{$field_info->name}-true' class='radio-uniform'  type='radio' name='{$field_info->name}' value='A' $checked /> ".$this->default_true_false_text[1]."</label> ";
		
		$checked = $value === 'I' || ($value_is_null && $field_info->default === 'I') ? "checked = 'checked'" : ""; 
		$input .= "<label><input id='field-{$field_info->name}-false' class='radio-uniform' type='radio' name='{$field_info->name}' value='I' $checked /> ".$this->default_true_false_text[0]."</label>";
